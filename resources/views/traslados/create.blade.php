        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <div id="personas-section" style="display:none;">
                    ...
                    <button type="button" class="btn-modern btn-save mb-2" onclick="agregarPersona()">Agregar Persona</button>
                ...
                <div id="materiales-section" style="display:none;">
                    ...
                    <button type="button" class="btn-modern btn-save mb-2" onclick="abrirModalMateriales()">Agregar Material</button>
                    <div class="table-responsive">
...
<style>
    ...
    .btn-save { background: #4cd137; color: #fff; }
    .btn-save:hover { background: #43a047; transform: translateY(-2px); box-shadow: 0 4px 10px rgba(76, 209, 55, 0.4); }

    .btn-cancel { background-color: #fff; border: 1px solid #43a047; color: #43a047; }
    .btn-cancel:hover { background: #43a047; color: #fff; transform: translateY(-2px); }
</style>
@endpush
...
function cargarMateriales(page = 1, search = '') {
    ...
    tbody.innerHTML = `<tr><td colspan="5" class="text-center"><div class="spinner-border text-success" role="status"><span class="visually-hidden">Cargando...</span></div></td></tr>`;
    ...
    .then(data => {
        ...
        if (data.data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted py-4"><i class="fas fa-inbox fa-2x mb-2"></i><p>No se encontraron materiales</p></td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = `<tr><td colspan="5" class="text-center text-danger"><i class="fas fa-exclamation-triangle"></i> Error al cargar materiales</td></tr>`;
    });
}

function seleccionarMaterial(event, id, nombre, tipo){
    event.stopPropagation();
    if(materialesSeleccionados.includes(id)) { alert('Este material ya ha sido agregado'); return; }
...
function renderizarPaginacion(data){
    ...
    for(let i=1;i<=data.last_page;i++){
        if(i===1 || i===data.last_page || (i>=data.current_page-1 && i<=data.current_page+1)){
            const li=document.createElement('li'); li.className=`page-item ${i===data.current_page?'active':''}`;
    ...
    const nextLi=document.createElement('li'); nextLi.className=`page-item ${data.current_page===data.last_page?'disabled':''}`;
...
function eliminarFila(button){ button.closest('tr').remove(); }
