    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="content-card">
                ...
                {{-- ✅ Componente Centro y Sede con valores pre-cargados para edición --}}
                <x-centros-sedes-selector
                    :centros="$centros"
                    :required="true"
                    :centroId="old('centro_id', $inventory->sede->centro_id ?? null)"
                    :sedeId="old('sede_id', $inventory->sede_id)"
                    :centroNombre="old('centro_id') ? '' : ($inventory->sede->centro->nom_centro ?? '')"
                    :sedeNombre="old('sede_id') ? '' : ($inventory->sede->nom_sede ?? '')"
                />
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-modern align-middle" id="materialsTable">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <input type="text" name="materials[{{ $i }}][material_name]"
                                class="form-control modern-input-sm" value="{{ $material->material_name }}"
                                placeholder="Nombre" required>
                        </td>
                        <td>
                            <input type="number" name="materials[{{ $i }}][material_quantity]"
                                class="form-control modern-input-sm" value="{{ $material->material_quantity }}"
                                placeholder="0" required>
                        </td>
                        <td>
                            <select name="materials[{{ $i }}][material_type]" class="form-select modern-input-sm">
                                <option value="Consumible" {{ $material->material_type == 'Consumible' ? 'selected' : '' }}>Consumible</option>
                                <option value="Herramienta" {{ $material->material_type == 'Herramienta' ? 'selected' : '' }}>Herramienta</option>
                            </select>
                        </td>
                        <td>
                            <input type="number" name="materials[{{ $i }}][material_price]"
                                class="form-control modern-input-sm" value="{{ $material->material_price }}"
                                step="0.01" placeholder="0.00" required>
                        </td>
                        <td>
                            <select name="materials[{{ $i }}][iva_percentage]" class="form-select modern-input-sm">
                        ...
                        <td>
                            <input type="text" name="materials[{{ $i }}][total_without_tax]"
                                class="form-control modern-input-sm bg-light" value="{{ $material->total_without_tax }}" readonly>
                        </td>
                        <td>
                            <input type="text" name="materials[{{ $i }}][total_with_tax]"
                                class="form-control modern-input-sm bg-light" value="{{ $material->total_with_tax }}" readonly>
                        </td>
                        <td>
                            <input type="text" name="materials[{{ $i }}][observations]"
...
<style>
    ...
    .table-modern input,
    .table-modern select { text-align: center; min-width: 100px; }
    ...
    .btn-light:hover { background: #f8f9fa; border-color: #adb5bd; }
    ...
    @media (max-width: 768px) {
        ...
        .card-header-custom i { font-size: 20px; }
    }
</style>
@endpush
...
<script>
    ...
    function updateMaterialTotals() {
        document.querySelectorAll('#materialsTable tbody tr').forEach(row => {
            const qty   = parseFloat(row.querySelector('[name*="[material_quantity]"]')?.value) || 0;
            const price = parseFloat(row.querySelector('[name*="[material_price]"]')?.value)    || 0;
            const iva   = parseFloat(row.querySelector('[name*="[iva_percentage]"]')?.value)    || 0;

            const withoutTax = qty * price;
            const withTax    = withoutTax + (withoutTax * iva / 100);

            const wtOut = row.querySelector('[name*="[total_without_tax]"]');
            const wtIn  = row.querySelector('[name*="[total_with_tax]"]');
            if (wtOut) wtOut.value = withoutTax.toFixed(2);
            if (wtIn)  wtIn.value  = withTax.toFixed(2);
        });
    }
    ...
    // Preview de imagen al seleccionar nueva
    document.getElementById('fileInput').addEventListener('change', function (e) {
        const file = e.target.files[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = e => {
            const previewBox = document.querySelector('.image-preview-box img');
            if (previewBox) previewBox.src = e.target.result;
        };
