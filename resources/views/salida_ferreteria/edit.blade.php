            ...
            <div class="col-12 col-md-6">
                <div class="content-card mb-4">
                        ...
                        <select name="user_id" class="form-select modern-input" required>
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ old('user_id', $salidaFerreteria->user_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <x-centros-sedes-selector
                        :centros="$centros"
                        :required="true"
                        :centroId="old('centro_id', $salidaFerreteria->centro_id)"
                        :sedeId="old('sede_id', $salidaFerreteria->sede_id)"
                        :centroNombre="old('centro_id') ? '' : ($salidaFerreteria->centro->nom_centro ?? '')"
                        :sedeNombre="old('sede_id') ? '' : ($salidaFerreteria->sede->nom_sede ?? '')"
                    />
...
<style>
    ...
    @media (max-width: 768px) {
        .salida-ferreteria-edit .section-header { flex-direction: column; align-items: flex-start; gap: 15px; }
        .salida-ferreteria-edit .col-md-6 { width: 100%; }
        .salida-ferreteria-edit .form-footer { flex-direction: column; gap: 10px; }
        .salida-ferreteria-edit .form-footer .btn { width: 100%; margin: 0 !important; }
    }
    #centroModal .btn,
#sedeModal .btn {
...
<script>
    ...
    function removeMaterial(button) {
        if (!confirm('¿Eliminar este material de la salida?')) return;
    ...
    document.querySelectorAll('#salidaForm input, #salidaForm select, #salidaForm textarea')
        .forEach(el => el.addEventListener('change', () => formChanged = true));

    window.addEventListener('beforeunload', e => {
        if (formChanged) { e.preventDefault(); e.returnValue = ''; }
    });

    document.getElementById('salidaForm').addEventListener('submit', () => formChanged = false);
</script>
@endpush